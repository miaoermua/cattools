...
catwrt_opkg_list_installed() {
    ...
    if [ $? -ne 0 ]; then
        echo "[Error] 无法从 API 服务器获取到支持的软件包数据"
        exit 1
    fi
    ...
    main() {
        if grep -q "repo.miaoer.xyz" /etc/opkg/distfeeds.conf; then
            echo "Sponsor us    ==========================================================="
            echo "你执行的下面命令使用的主站软件源 repo.miaoer.xyz 将对服务器带宽产生新的挑战"
